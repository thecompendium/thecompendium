...
const News: React.FC<NewsProps> = ({ publications, isAdmin, setPublications }) => {
  ...
  return (
    <div className="pt-32 pb-32 px-6 bg-[#000b1a] min-h-screen">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="text-left mb-16 border-b border-white/10 pb-12">
          <div className="flex flex-col lg:flex-row justify-between items-end gap-10">
             <div>
                <h1 className="text-5xl font-bold text-white mb-4 serif-font">Newsroom & Archives</h1>
                <p className="text-xl text-gray-400 font-light max-w-2xl">A curated platform for intellectual dialogue and campus updates.</p>
             </div>
             <div className="flex gap-4">
                {isAdmin && <button onClick={handleOpenAdd} className="px-8 py-3 bg-yellow-400 text-black font-black rounded-xl text-[10px] uppercase tracking-widest shadow-xl">+ New Publication</button>}
                <select value={sortBy} onChange={e => setSortBy(e.target.value as SortOption)} className="bg-white/5 border border-white/10 text-white rounded-xl px-5 py-3 text-[11px] font-bold uppercase tracking-widest focus:outline-none">
          ...
          <div className="flex flex-wrap gap-2 mt-12">
            {['All', 'Article', 'College News', 'Annual Magazine'].map(cat => (
              <button key={cat} onClick={() => setActiveFilter(cat)} className={`px-5 py-2 rounded-full text-[11px] font-black uppercase tracking-widest transition-all border ${activeFilter === cat ? 'bg-yellow-400 text-black border-yellow-400 shadow-xl' : 'bg-transparent text-gray-400 border-white/10 hover:text-white'}`}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
          {filteredAndSortedPublications.map((pub) => (
            <div key={pub.id} className="group bg-[#0d121f] rounded-2xl overflow-hidden flex flex-col h-full text-left shadow-2xl transition-all hover:-translate-y-2 border border-white/5">
              {isAdmin && (
              ...
              <div className="h-60 overflow-hidden relative cursor-pointer" onClick={() => pub.file_url && window.open(pub.file_url, '_blank')}>
                ...
                <div className="absolute top-5 left-5">
                  <span className="px-5 py-2 bg-[#001bb8] text-white text-[10px] font-black rounded-full uppercase tracking-widest shadow-2xl border border-white/10">
                    {pub.category === 'Article' ? 'Articles' : pub.category}
                  </span>
                </div>
              </div>
              <div className="p-8 flex flex-col flex-grow">
                <h3 className="text-2xl font-bold mb-4 leading-tight text-white group-hover:text-yellow-400 transition-colors line-clamp-2 h-[4rem] serif-font">
                  {pub.title}
                </h3>
                <p className="text-base text-gray-400 mb-8 line-clamp-3 leading-relaxed flex-grow font-light">
                  {pub.summary}
                </p>
                <button onClick={() => pub.file_url && window.open(pub.file_url, '_blank')} className="text-[11px] font-black text-white hover:text-yellow-400 uppercase tracking-[0.3em] transition-all flex items-center gap-3">
                  Read Full Piece <span className="text-xl">→</span>
                </button>
              </div>
            </div>
          ))}
